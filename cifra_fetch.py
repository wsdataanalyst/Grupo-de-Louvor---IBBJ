...
def _extract_vagalume_cifra(html: str) -> str:
    ...
    if not m:
        m = re.search(r'<div id=lyrics[^>]*>(.*?)</div>', html, re.S | re.I)
    if not m:
        return ""
    return _clean_cifra_html(m.group(1))
...
def fetch_from_vagalume(
    ...
    *,
    ...
    for artist_slug in artist_slugs:
        ...
        try:
            if not out.cifra_text:
                html_cifra = _http_get(cifrada_url)
                out.cifra_text = _extract_vagalume_cifra(html_cifra)
                if out.cifra_text:
                    out.cifra_url = cifrada_url
        except (urllib.error.URLError, TimeoutError, OSError) as exc:
            logger.info("Vagalume cifra falhou %s: %s", cifrada_url, exc)
